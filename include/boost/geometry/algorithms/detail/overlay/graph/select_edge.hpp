...
private:
    ...
    turn_operation_id select_by_side(edges_type& edges, point_type const& p1, point_type const& p2) const
    {
        ...
        // For dissolve, the left turn is preferred to trace the outer boundary
        // of self-intersecting polygons (avoiding small inner loops).
        ...
        auto compare_one_side = [&](auto const& a, auto const& b) -> bool
        {
            ...
            if constexpr (OverlayType == overlay_dissolve)
            {
                // For dissolve, prefer leftmost edge
                return side == -1;
            }
            else
            {

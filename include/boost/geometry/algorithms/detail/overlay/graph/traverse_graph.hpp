...
struct traverse_graph
{
            ...
            signed_size_type target_node_id)
    {
        ...
        if constexpr (OverlayType == overlay_dissolve)
        {
            // For dissolve, add to both sets:
            // - m_finished_tois prevents re-starting from the same ops (avoids duplicate rings)
            // - m_all_traversed_tois is used for update_administration
            // Note: continue_traverse skips the m_finished_tois check for dissolve,
            // allowing ops to be "passed through" during ring traversal.
...
private:
    ...
    // For dissolve: all traversed tois across all rings (for update_administration only)

...
struct map_visitor
{
    ...
    void visit_cluster_connections(Args const&... ) {}

    template <typename Turns, typename Turn, typename Operation>
    void visit_traverse(Turns const& , Turn const& , Operation const& , char const*)
    {}

    template <typename Turns, typename Turn, typename Operation, typename ErrorType>
        ...
        ut_settings const& settings)
{
    ...
    // Check dispatch::dissolve
    {
        ...
        bg::detail::dissolve::dissolve_dummy_robust_policy robust_policy;
...
void test_all(ut_settings const& settings_for_sensitive_cases)
{
...
#if defined(BOOST_GEOMETRY_TEST_SELF_INTERSECTING_RINGS)
    // These tests have input Polygons with self-intersecting rings.
    // Self-intersecting rings are neither valid outer nor inner rings
    // (edge direction is mixed CW/CCW). The correct decomposition
    // is subjective and depends on the MakeValid strategy used.
    // Enable by defining BOOST_GEOMETRY_TEST_SELF_INTERSECTING_RINGS.
    ...
    if constexpr (! std::is_same_v<typename bg::coordinate_type<P>::type, float>)
    {
